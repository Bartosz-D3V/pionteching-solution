...
public class TransactionServiceImpl implements TransactionService {
    ...
    public Flowable<Account> processTransactions(Collection<Transaction> transactions) {
        ...
        return Flowable.fromIterable(transactions).reduce(new HashMap<String,Account>(), (acc, transaction) -> {
            ...
            if (!creditAccount.equals(debitAccount)) {
                ...
                acc.computeIfAbsent(creditAccount, (key) -> new Account(creditAccount, 0, 0, BigDecimal.ZERO.setScale(2, RoundingMode.HALF_UP)));
                acc.computeIfAbsent(debitAccount, (key) -> new Account(debitAccount, 0, 0, BigDecimal.ZERO.setScale(2, RoundingMode.HALF_UP)));
            }

            return acc;
        }).flattenAsFlowable(HashMap::values).sorted();
    }
}
